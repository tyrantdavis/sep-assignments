...
SELECT v.name FROM volunteers AS v
  (SELECT d.name FROM dogs) AS d
  WHERE v.id = d.id;

  --8b. List the adopter’s name and the pet’s name for each animal adopted within the past month to be
  ...
  SELECT adopters.name, dogs.name, cats.name
  FROM adopters, dogs, cats
    (SELECT date
    FROM adoptions
    WHERE adoptions.id = adopters.aid
    AND date >= '2017-11-15');
...
--8d. Display a list of all cats and all dogs who have not been adopted.
...
WHERE EXISTS
  ...
  OR cats.adoption_date = NULL);



--8e. Create a list of volunteers who are available to foster. If they currently are fostering a dog, include
...
SELECT adoptions.adopter, a.first_name, a.last_name
FROM adoptions, adopters
WHERE
  (SELECT id
    FROM cats
    WHERE adoptions.adopter = cats.id
    AND name = 'Seashell');
