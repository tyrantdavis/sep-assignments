...
FROM patrons, holds
  (SELECT isbn
  FROM books
  WHERE books.isbn = holds.isbn
  AND title = 'Harry Potter and the Sorcerer’s Stone')
GROUP BY name
ORDER BY rank;
...
SELECT DATE_PART ('DAY', t.checked_in_date - t.checked_out_date As DateDiff)
FROM transactions
 (SELECT isbn
 FROM books
 WHERE books.isbn = transactions.isbn)
...
FROM books
  ...
  FROM transactions
  WHERE checked_out_date <= '2012-12-15')
...
SELECT SUM(transactions.checked_out_date) AS books_checked_out
FROM transactions
  (SELECT name
  FROM patrons
  WHERE patrons.id = transactions.user_id)
GROUP BY SUM(transactions.checked_out_date)
HAVING checked_out_date >= 1 ;
